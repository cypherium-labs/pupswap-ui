...
import { mainnet, testnet } from "../../cypherium_address.json";
...
const BAD_RECIPIENT_ADDRESSES: string[] = [
  mainnet.factory, // v2 factory
  mainnet.routerv2, // v2 router 02
];
...
function parseCurrencyFromURLParameter(urlParam: any): string {
  if (typeof urlParam === "string") {
    ...
    if (valid) return urlParam; // Return the valid address
    if (urlParam.toUpperCase() === "CPH") return "CPH"; // Return ETH if the string is ETH
  }
  return "CPH"; // Default to "ETH" if urlParam is not a valid string or address
}
