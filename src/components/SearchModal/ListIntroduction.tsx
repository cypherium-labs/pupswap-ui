...
export default function ListIntroduction({
  ...
  const [isDark, _toggle] = useDarkModeManager();
      ...
      <PaddedColumn>
        <AutoColumn gap="14px">
          <img
            ...
            src={
              isDark
                ? ((listDark as unknown) as string)
                : ((listLight as unknown) as string)
            }
          ...
          <Text style={{ marginBottom: "8px", textAlign: "center" }}>
            DCTDEX supports token lists. You can add your own custom lists via
            IPFS, HTTPS and ENS.{" "}
          </Text>
          <ButtonPrimary
